				...
				for (int i=0; i < ParticleList.Length;i=i+divide_factor)
				{
					...
					if(counter < Gameobj_instances.Count){
						...
						if(end_of_life){
							...
							if(ParticleList[i].lifetime < life_factor_upper*ParticleList[i].startLifetime
							   & ParticleList[i].lifetime > life_factor_lower*ParticleList[i].startLifetime
			...
			if( (propagator_exists & Propagator_use_collisions & !Procedural_check) | !propagator_exists ){
				...
				if((propagator_exists &&(!Propagator_by_layer ||(Propagator_by_layer & layer_same ))) || !propagator_exists){
					                         ...
					                         )) || !propagator_exists)
					{
						...
						if(propagator_exists){
							particle_count = Propagator.p11.maxParticles;
						}
									...
									if(!Propagator.Erase_mode){
										...
										if(Propagator.Registered_paint_positions!=null ){
											...
											if(Propagator.Registered_paint_positions.Count > ( particle_count/2)){
												...
												if(collisionEvents[j][k].collider !=null){
													if(collisionEvents[j][k].collider.gameObject !=null){
														...
														if(is_close_to_other_point_on_object == 1){
															...
															if(!Propagator.gameobject_mode){
																Propagator.Registered_paint_size.Add (Propagator.p11.startSize+Propagator.Random_size_factor);
															}
															...
															Propagator.Emitter_objects.Add(collisionEvents[j][k].collider.gameObject.transform);
															Propagator.Registered_initial_positions.Add(collisionEvents[j][k].collider.gameObject.transform.position);
															Propagator.Registered_initial_scale.Add (collisionEvents[j][k].collider.gameObject.transform.localScale);
															
															Propagator.Registered_initial_rotation.Add(collisionEvents[j][k].collider.gameObject.transform.rotation);
