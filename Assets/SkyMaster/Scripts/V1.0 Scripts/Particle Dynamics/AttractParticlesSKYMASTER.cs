	...
	{	
	  ...
	  if(Application.isPlaying | Enable_Preview){
		...
		if(MultiThreaded){
							...
							if(p11.simulationSpace == ParticleSystemSimulationSpace.World){
								World_sim=true;
							}
									...
									for (int i=0; i < (ParticleList.Length/2);i++)
									{
										...
										if (dist < sqrDist) {
											...
											if(Turbulance){ //Version 1.2 ADDITION
												...
												if(vortex_motion & p11_tag != "VortexSafe" & p11_tag != "Grass"){
													...
													Vortexes[count_active].active = true;
														...
														ParticleList[i].lifetime = Vortex_life;
													...
													if(j< Vortexes.Count & Vortexes[j].active){
														if(Vortexes[j].particle_ID == i){
															...
															if(Show_vortex){
																ParticleList[i].size = Vortex_center_size;
																ParticleList[i].color = Vortex_center_color;
															}
											...
											if(Color_force){	//Version 1.2 ADDITION
												...
												ParticleList[i].color = Color.Lerp(ParticleList[i].color, Force_color,0.1f);
												
											}
													//v1.6
													if(End_LifeOf_Affected){
														ParticleList[i].lifetime=End_Life;
													}
										...
										if(splash_effect){
											if(ParticleList[i].lifetime < 0.05f){
												ParticleList[i].lifetime = ParticleList[i].startLifetime;
												...
												if(vary_turbulence){				
													...
													if(splash_noise){
														if(ParticleList[i].lifetime < splash_noise_tres){
			...
			if (make_moving_star & p11.particleCount > 2 & repel & OBJ_DIST<star_trail_dist){
				for (int i=0; i<star_trails; i++) {
					...
					for (int j = 0;  j < (p11.particleCount/star_trails);  j++) {
						...
						ParticleList[countit].size = ParticleList[countit].size - j * size_of_trail;
