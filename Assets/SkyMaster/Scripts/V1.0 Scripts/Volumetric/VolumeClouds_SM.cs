...
public class VolumeClouds_SM : MonoBehaviour {
		...
		//v3.0-alt clouds
		[HideInInspector]
		public bool Use_quads = false;//instantiate quads for clouds
		[HideInInspector]
...
//				divider -= 1;//make even
//			}
			int psize = (int)GetComponent<ParticleEmitter> ().minEmission;
				...
				if(is_DayLight ){
					...
					if(is_after_17 ){
						...
						if(is_before_10){
							...
							if(!Grab_sun_color){
								SunColor = Color.Lerp(SunColor,Dawn_sun_col,Trans_speed);
							}
...
//								}
								...
								if(StableRollMethod2){
									...
									if(diffRot == 0 && currentRotY!=Prev_cam_rot.y && Mathf.Abs(Vector3.Angle(Cam_transf.forward, Vector3.up))< 65){//Mathf.Abs(Vector3.Angle(Cam_transf.forward, Vector3.up))< 65){
									
										float diffRotY = currentRotY - Prev_cam_rot.y;
										particles[i].rotation = particles[i].rotation+diffRotY;
										//Debug.Log("aa");
									}else{
										if(Mathf.Abs(Vector3.Angle(Cam_transf.forward, Vector3.up)) < 45){
											...
											particles[i].rotation = particles[i].rotation+diffRot;
										}
									}
								}
