...
[ExecuteInEditMode()]
	...
	void Update () {
			...
			if( nbb.maxParticles < particle_count_init & Application.isPlaying){
				particle_count = nbb.maxParticles;
			}else if( nbb.maxParticles > particle_count_init & Application.isPlaying){
		...
		for(int i=0;i<aaa.Length;i++){
		  if(i < tile.Length){
					...
					if(keep_alive){
			aaa[i].lifetime = tileCount + 1 - tile[i];
