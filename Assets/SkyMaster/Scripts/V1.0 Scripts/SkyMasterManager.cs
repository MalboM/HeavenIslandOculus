...
namespace Artngame.SKYMASTER {
	...
	public class WeatherSM {
		...
		public ParticleEmitter VolumeCloudemitter;
			...
			if (currentState == Volume_Weather_State.FadeIn) {
				...
				if (Snow) {
					for (int i=0; i<SnowParticle.Count; i++) {
						Make_Particles_Appear (SnowParticle [i].gameObject, SnowParticleP [i], null, SkyManager.Snow_max_part, true, 10, 1);
					}
				...
				if (Rain) {
						if(Refractive_Rain){
							for (int i=0; i<RefractRainParticle.Count; i++) {
								Make_Particles_Appear (RefractRainParticle[i].gameObject, RefractRainParticleP[i], null, SkyManager.Refract_Rain_max_part, true, 10, 1);
							}
						}else{
							for (int i=0; i<RainParticle.Count; i++) {
								Make_Particles_Appear (RainParticle[i].gameObject, RainParticleP[i], null, SkyManager.Refract_Rain_max_part, true, 10, 1);
							}
						}
				}else{
						for (int i=0; i<RefractRainParticle.Count; i++) {
			...
			if(Particle_OBJ!=null){
				if(!Particle_OBJ.activeInHierarchy){
					...
					if(reset_pcount){
						if(ParticleOBJ_Ps!=null){
							for (int n = 0; n < ParticleOBJ_Ps.Length; n++){
								if(ParticleOBJ_Ps[n].maxParticles < max_particles){
									ParticleOBJ_Ps[n].maxParticles += rate; // RESTORE max from spring elimination
									//v2.2									 
									if(ParticleOBJ_Ps[n].startColor.a < Transp){									
										ParticleOBJ_Ps[n].startColor = new Color(ParticleOBJ_Ps[n].startColor.r,ParticleOBJ_Ps[n].startColor.g,ParticleOBJ_Ps[n].startColor.b,ParticleOBJ_Ps[n].startColor.a+0.02f*rate*Time.deltaTime);
									}
								}
						...
						if(ParticleOBJ_P!=null){
							if(ParticleOBJ_P.maxParticles < max_particles){
								ParticleOBJ_P.maxParticles += rate; // RESTORE max from spring elimination
							}
						...
						if(ParticleOBJ_Ps!=null){
							for (int n = 0; n < ParticleOBJ_Ps.Length; n++){
								ParticleOBJ_Ps[n].maxParticles = max_particles; // RESTORE max from spring elimination
								ParticleOBJ_Ps[n].Play();
							}
						}
						if(ParticleOBJ_P!=null){
							ParticleOBJ_P.maxParticles = max_particles; // RESTORE max from spring elimination
							ParticleOBJ_P.Play();
						}
					}
				}
			...
			if(Particle_OBJ!=null){
				if(Particle_OBJ.activeInHierarchy){
					if(ParticleOBJ_Ps!=null){
						...
						for (int n = 0; n < ParticleOBJ_Ps.Length; n++){
							ParticleOBJ_Ps[n].maxParticles = ParticleOBJ_Ps[n].maxParticles-rate;							
							if(pull_downward){
								ParticleOBJ_Ps[n].gravityModifier = -10;
							}							
							if(ParticleOBJ_Ps[n].maxParticles < min_particles){ //start increasing opaque
								ParticleOBJ_Ps[n].Stop();
								ParticleOBJ_Ps[n].Clear();
								if(on_lowest){
					...
					if(ParticleOBJ_P!=null){
						ParticleOBJ_P.maxParticles = ParticleOBJ_P.maxParticles-rate;	
						
						if(pull_downward){
							ParticleOBJ_P.gravityModifier = -10;
						}						
						if(ParticleOBJ_P.maxParticles < min_particles){ //start increasing opaque
							ParticleOBJ_P.Stop();
							ParticleOBJ_P.Clear();
							Particle_OBJ.SetActive(false);
						}
...
public class SkyMasterManager : MonoBehaviour 
{
		...
		public float max_moon_intensity=1;
		...
		void LateUpdate(){
			...
			//if ((currentWeatherName != prevWeatherName && ((currentWeather != null && currentWeather.currentState != WeatherSM.Volume_Weather_State.FadeIn) | currentWeather == null )) || currentWeather == null) {	
				...
				if(currentWeatherName == Volume_Weather_types.Cloudy | currentWeatherName == Volume_Weather_types.FlatClouds){
					...
					Vector3 VolCloudStartPos = MapCenter.position + new Vector3(0,VolCloudHeight,0) - Along_wind_vector;
...
//					}
		...
		//v.2.0.1
		public float Max_sun_intensity = 0.67f;
	...
	Vector4 mGroundColor;
		...
		public Color Spring_fog_dusk=new Color(0.1f,0.06f,0.1f,1);
...
//			{}
//			else{
//				ORK.Game.Variables.Set("TIME_DAY",0);
...
//		KEEP_START_FOG_DENSITY = RenderSettings.fogDensity;
	...
	void Assign_Material_Props(Material mat, Transform CameraTransform)
	{
		...
		float fKm4PI = m_Km*4.0f*Mathf.PI;
			...
			Vector3 cam_pos1 = CameraTransform.position;
		...
		if(USE_SKYCUBE){
			mat.SetVector("v3CameraPos",  SkyCam_transform.localPosition);
				...
				if(Unity5){
					InitCam = new Vector3(62.67624f,10016,107.306f);
					Vector3 cam_pos = CameraTransform.position;
					if(Camera.current != null & Application.isEditor){
			...
			if(!alter_sun){
				...
				//v3.0
				bool Check_time  = !AutoSunPosition && ( (Current_Time > (22.4 + Shift_dawn) & Current_Time <=25) | (Current_Time >= 0 & Current_Time < (9 + Shift_dawn) ));
				bool Check_angle = AutoSunPosition && Rot_Sun_X <= 0;
				//v1.7
				...
				if(Check_time | Check_angle){
					...
					mat.SetVector("v3LightDir", (Vector3.one*Sun_ring_factor)-(CameraTransform.position-MoonObj.transform.position).normalized);
					...
					mat.SetVector("v3LightDir", (Vector3.one*Sun_ring_factor)-(CameraTransform.position-SunObj.transform.position).normalized);
				...
				if(SunObj2 !=null){
					...
					mat.SetVector("v3LightDirMoon", (Vector3.one*Sun_ring_factor)-(CameraTransform.position-SunObj2.transform.position).normalized);
				}
...
//			}
		...
		{			
			...
			//float CURRENT_DAY = 0; //ORK2

			if(USE_ORK){

//				CURRENT_TIME = ORK.Game.Variables.GetFloat("TIME"); //ORK2
//				CURRENT_DAY = ORK.Game.Variables.GetFloat("TIME_DAY"); //ORK2
//				//float CURRENT_TIME = GameHandler.GetNumberVariable("TIME"); //ORK1
//				//float CURRENT_DAY = GameHandler.GetNumberVariable("TIME_DAY"); //ORK1
//				
//				//if (CURRENT_TIME < 24*60*60){ // count secs of 24 hours
//				if (CURRENT_TIME < 24){
//					ORK.Game.Variables.Set("TIME",CURRENT_TIME+0.001f*SPEED); 
//					//GameHandler.SetNumberVariable("TIME",CURRENT_TIME+0.001f*SPEED); //ORK1
//				}
//				else{
//					ORK.Game.Variables.Set("TIME",0); 
//					//GameHandler.SetNumberVariable("TIME",0); //ORK1
//					
//					//UPDATE DAY
//					if (ORK.Game.Variables.GetFloat("TIME_DAY") != null)
//						//if (GameHandler.GetNumberVariable("TIME_DAY") != null) //ORK1
...
//							//GameHandler.SetNumberVariable("TIME_DAY",GameHandler.GetNumberVariable("TIME_DAY")+1); //ORK1
...
//				}
			...
			///TOD
				...
				//	Debug.Log("Day ="+Day_numberN);
					...
					float Fraction_YearG = (360/365f)*(Day_numberN + (Current_Time)/24)*Mathf.Deg2Rad;
				...
				//	Debug.Log("TimeCorrecSolarAngTC = "+TimeCorrecSolarAngTC);
					float SolarHourAngleSHA = ((Current_Time) - 12)*15+ LonCoord + TimeCorrecSolarAngTC;
						...
						if(CURRENT_YEAR_QUARTER > 3.1f & CURRENT_YEAR_QUARTER < 3.65f ){
							...
							if(SnowStorm_OBJ != null){
								if(!SnowStorm_OBJ.activeInHierarchy){
									...
									if(SnowStorm_P!=null){
										for (int n = 0; n < SnowStorm_P.Length; n++){
											SnowStorm_P[n].maxParticles = Snow_cloud_max_part; // RESTORE max from spring elimination
											SnowStorm_P[n].Play();
										}
									}
								}
							...
							if(SnowStorm_OBJ != null){
								if(SnowStorm_OBJ.activeInHierarchy){
									if(SnowStorm_P!=null){
										for (int n = 0; n < SnowStorm_P.Length; n++){
											SnowStorm_P[n].maxParticles = SnowStorm_P[n].maxParticles-80;								
											if(SnowStorm_P[n].maxParticles < 5){ //start increasing opaque
												SnowStorm_P[n].Stop();
												SnowStorm_P[n].Clear();
												SnowStorm_OBJ.SetActive(false);
												Ice_Spread_OBJ.SetActive(false);
							...
							if(FallingLeaves_OBJ[o].activeInHierarchy){
								if(FallingLeaves_P!=null){
									for (int n = 0; n < FallingLeaves_P.Length; n++){
										FallingLeaves_P[n].maxParticles = FallingLeaves_P[n].maxParticles-10;								
										if(FallingLeaves_P[n].maxParticles < 5){ //start increasing opaque
...
//						}
						...
						if(CURRENT_YEAR_QUARTER >=3.4f & CURRENT_YEAR_QUARTER < 3.9f){
							...
							if(!Surround_Clouds_Heavy.activeInHierarchy){
								...
								if(Surround_Clouds_Heavy_P!=null){
									for (int n = 0; n < Surround_Clouds_Heavy_P.Length; n++){
										Surround_Clouds_Heavy_P[n].maxParticles = Storm_cloud_max_part; // RESTORE max from spring elimination
							...
							if(!Rain_Heavy.activeInHierarchy){
								...
								if(Heavy_Rain_P!=null){
									for (int n = 0; n < Heavy_Rain_P.Length; n++){
										Heavy_Rain_P[n].maxParticles = 300; // RESTORE max from spring elimination
										Heavy_Rain_P[n].Play();
									}
								}
							}
								...
								if(Lower_Cloud_Bed.activeInHierarchy){
									if(Cloud_bed_Dn_P!=null){
										for (int n = 0; n < Cloud_bed_Dn_P.Length; n++){
											Cloud_bed_Dn_P[n].maxParticles = Cloud_bed_Dn_P[n].maxParticles-110;								
											if(Cloud_bed_Dn_P[n].maxParticles < 5){ //start increasing opaque
							...
							if(Lower_Cloud_Bed != null){
								if(!Lower_Cloud_Bed.activeInHierarchy){
									...
									if(Cloud_bed_Dn_P!=null){
										for (int n = 0; n < Cloud_bed_Dn_P.Length; n++){
											Cloud_bed_Dn_P[n].maxParticles = Flat_cloud_max_part; // RESTORE max from spring elimination
						...
						if(!Upper_Cloud_Bed.activeInHierarchy){
							...
							if(Cloud_bed_Up_P!=null){
								for (int n = 0; n < Cloud_bed_Up_P.Length; n++){
									Cloud_bed_Up_P[n].maxParticles = Flat_cloud_max_part; // RESTORE max from spring elimination
							...
							if(Lower_Cloud_Real.activeInHierarchy){
								if(Real_Clouds_Dn_P!=null){
									for (int n = 0; n < Real_Clouds_Dn_P.Length; n++){
										Real_Clouds_Dn_P[n].maxParticles = Real_Clouds_Dn_P[n].maxParticles-10;								
										if(Real_Clouds_Dn_P[n].maxParticles < 5){ //start increasing opaque
							...
							if(Upper_Cloud_Real.activeInHierarchy){
								if(Real_Clouds_Up_P!=null){
									for (int n = 0; n < Real_Clouds_Up_P.Length; n++){
										Real_Clouds_Up_P[n].maxParticles = Real_Clouds_Up_P[n].maxParticles-10;								
										if(Real_Clouds_Up_P[n].maxParticles < 5){ //start increasing opaque
						...
						if(Enable_fog){
							...
							FogMode Fog_Mode = FogMode.ExponentialSquared;
						...
						if(CURRENT_YEAR_QUARTER >=2.0f){
							for (int o = 0; o < FallingLeaves_OBJ.Length; o++){
								if(!FallingLeaves_OBJ[o].activeInHierarchy){
									...
									if(FallingLeaves_P!=null){
										for (int n = 0; n < FallingLeaves_P.Length; n++){
											FallingLeaves_P[n].maxParticles = 300; // RESTORE max from spring elimination
...
//							}
						...
						if(Lower_Cloud_Real != null){
							if(!Lower_Cloud_Real.activeInHierarchy){
								...
								if(Real_Clouds_Dn_P!=null){
									for (int n = 0; n < Real_Clouds_Dn_P.Length; n++){
										Real_Clouds_Dn_P[n].maxParticles = Real_cloud_max_part; // RESTORE max from spring elimination
						...
						if(Upper_Cloud_Real != null){
							if(!Upper_Cloud_Real.activeInHierarchy){
								...
								if(Real_Clouds_Up_P!=null){
									for (int n = 0; n < Real_Clouds_Up_P.Length; n++){
										Real_Clouds_Up_P[n].maxParticles = Real_cloud_max_part; // RESTORE max from spring elimination
						...
						if(CURRENT_YEAR_QUARTER >=2.5f){
							if(Surround_Clouds != null){
								if(!Surround_Clouds.activeInHierarchy){
									...
									if(Surround_Clouds_P!=null){
										for (int n = 0; n < Surround_Clouds_P.Length; n++){
											Surround_Clouds_P[n].maxParticles = 300; // RESTORE max from spring elimination
							...
							if(Lower_Dynamic_Cloud.activeInHierarchy){
								if(Dynamic_Clouds_Dn_P!=null){
									for (int n = 0; n < Dynamic_Clouds_Dn_P.Length; n++){
										Dynamic_Clouds_Dn_P[n].maxParticles = Dynamic_Clouds_Dn_P[n].maxParticles-10;								
										if(Dynamic_Clouds_Dn_P[n].maxParticles < 5){ //start increasing opaque
							...
							if(Upper_Dynamic_Cloud.activeInHierarchy){
								if(Dynamic_Clouds_Up_P!=null){
									for (int n = 0; n < Dynamic_Clouds_Up_P.Length; n++){
										Dynamic_Clouds_Up_P[n].maxParticles = Dynamic_Clouds_Up_P[n].maxParticles-10;								
										if(Dynamic_Clouds_Up_P[n].maxParticles < 5){ //start increasing opaque
						...
						if(Lower_Cloud_Bed != null){
							if(Lower_Cloud_Bed.activeInHierarchy){
								if(Cloud_bed_Dn_P!=null){
									for (int n = 0; n < Cloud_bed_Dn_P.Length; n++){
										Cloud_bed_Dn_P[n].maxParticles = Cloud_bed_Dn_P[n].maxParticles-10;								
										if(Cloud_bed_Dn_P[n].maxParticles < 5){ //start increasing opaque
											Cloud_bed_Dn_P[n].Stop();
											Cloud_bed_Dn_P[n].Clear();
											Lower_Cloud_Bed.SetActive(false);
						...
						if(Upper_Cloud_Bed.activeInHierarchy){
							if(Cloud_bed_Up_P!=null){
								for (int n = 0; n < Cloud_bed_Up_P.Length; n++){
									Cloud_bed_Up_P[n].maxParticles = Cloud_bed_Up_P[n].maxParticles-10;								
									if(Cloud_bed_Up_P[n].maxParticles < 5){ //start increasing opaque
										Cloud_bed_Up_P[n].Stop();
										Cloud_bed_Up_P[n].Clear();
										Upper_Cloud_Bed.SetActive(false);
									}
							...
							if(Butterfly_OBJ.activeInHierarchy){
								if(Butterflies2D_P!=null){
									for (int n = 0; n < Butterflies2D_P.Length; n++){
										Butterflies2D_P[n].maxParticles = Butterflies2D_P[n].maxParticles-40;								
										if(Butterflies2D_P[n].maxParticles < 5){ //start increasing opaque
						...
						if(Butterfly_OBJ != null){
							if(!Butterfly_OBJ.activeInHierarchy){							
								...
								if(Butterflies2D_P!=null){
									for (int n = 0; n < Butterflies2D_P.Length; n++){
										Butterflies2D_P[n].startColor = new Color(Butterflies2D_P[n].startColor.r,Butterflies2D_P[n].startColor.g,Butterflies2D_P[n].startColor.b,0);
									}
								}
							}else{ //start increasing opaque
								if(Butterflies2D_P!=null){
									for (int n = 0; n < Butterflies2D_P.Length; n++){
										Color START_C = Butterflies2D_P[n].startColor;
										Butterflies2D_P[n].startColor = Color.Lerp(START_C, new Color(START_C.r,START_C.g,START_C.b,1),0.5f*Time.deltaTime);
									}
								}						
							}
						}
						...
						if(CURRENT_YEAR_QUARTER > 0.6f){
							...
							if(Surround_Clouds.activeInHierarchy){
								...
								if(Surround_Clouds_P!=null){
									//Debug.Log ("ASER3");
									for (int n = 0; n < Surround_Clouds_P.Length; n++){
										Surround_Clouds_P[n].maxParticles = Surround_Clouds_P[n].maxParticles-20; //Debug.Log ("ASER4");								
										if(Surround_Clouds_P[n].maxParticles < 5){ //start increasing opaque
											Surround_Clouds_P[n].Stop();
											Surround_Clouds_P[n].Clear();
											Surround_Clouds.SetActive(false);
										}
						...
						if(CURRENT_YEAR_QUARTER >=0.4f){
							...
							if(Surround_Clouds_Heavy.activeInHierarchy){
								if(Surround_Clouds_Heavy_P!=null){
									for (int n = 0; n < Surround_Clouds_Heavy_P.Length; n++){
										Surround_Clouds_Heavy_P[n].maxParticles = Surround_Clouds_Heavy_P[n].maxParticles-10;								
										if(Surround_Clouds_Heavy_P[n].maxParticles < 5){ //start increasing opaque
											Surround_Clouds_Heavy_P[n].Stop();
											Surround_Clouds_Heavy_P[n].Clear();
											Surround_Clouds_Heavy.SetActive(false);
										}
							...
							if(Rain_Heavy.activeInHierarchy){
								if(Heavy_Rain_P!=null){
									for (int n = 0; n < Heavy_Rain_P.Length; n++){
										Heavy_Rain_P[n].maxParticles = Heavy_Rain_P[n].maxParticles-10;								
										if(Heavy_Rain_P[n].maxParticles < 5){ //start increasing opaque
											Heavy_Rain_P[n].Stop();
											Heavy_Rain_P[n].Clear();
											Rain_Heavy.SetActive(false);
										}
			...
			//if(Current_Time >= (9 + Shift_dawn) & Current_Time <= (22.4f + Shift_dawn)  ){
					...
					if(intensity1 != max_moon_intensity){
						MOON_LIGHT.GetComponent<Light>().intensity = Mathf.Lerp(intensity1,max_moon_intensity,Time.deltaTime);
					}
			}
			//MOON PHASES
			if(MoonPhases && MoonPhasesMat != null){
			...
			if(CloudDomeL1Mat != null & CloudDomeL2Mat != null){
				...
				ShaderCloudDomeCoverage2 = CloudDomeL2Mat.GetFloat("_CloudCover");
					...
					//CloudDomeL1Mat.SetVector("_CloudSpeed",new Vector4(-windZone.transform.forward.x, -windZone.transform.forward.z,0,0));
			}
			...
			if(Current_Time < 16 & Current_Time > 9 ){
					...
					if(!Mobile){
						...
						if(Application.isPlaying){
							if(Real_Clouds_Dn_P!=null){
							for (int n = 0; n < Real_Clouds_Dn_P.Length; n++){
								Real_Clouds_Dn_P[n].startColor = Color.Lerp(Real_Clouds_Dn_P[n].startColor,new Color(1,1f,1f,0.05f),0.5f*Time.deltaTime);
							}
							}
							if(Real_Clouds_Up_P!=null){
							for (int n = 0; n < Real_Clouds_Up_P.Length; n++){
								Real_Clouds_Up_P[n].startColor = Color.Lerp(Real_Clouds_Up_P[n].startColor,new Color(1,1f,1f,0.05f),0.5f*Time.deltaTime);
							}
							}
						}
					}
				...
				if(Current_Time > 17){ //BREAK stars near morning
					...
					if(!Mobile){
						...
						if(Application.isPlaying){
							if(Real_Clouds_Dn_P!=null){
							for (int n = 0; n < Real_Clouds_Dn_P.Length; n++){
								Real_Clouds_Dn_P[n].startColor = Color.Lerp(Real_Clouds_Dn_P[n].startColor,new Color(0,0.0f,0.0f,0.05f),0.5f*Time.deltaTime);
							}
							}
							if(Real_Clouds_Up_P!=null){
							for (int n = 0; n < Real_Clouds_Up_P.Length; n++){
								Real_Clouds_Up_P[n].startColor = Color.Lerp(Real_Clouds_Up_P[n].startColor,new Color(0,0.0f,0.0f,0.05f),0.5f*Time.deltaTime);
							}
							}
						}
					}
				...
				if(Current_Time >10.5){ //BREAK stars near morning
					...
					if(!Mobile){
						...
						if(Application.isPlaying){
							if(Real_Clouds_Dn_P!=null){
							for (int n = 0; n < Real_Clouds_Dn_P.Length; n++){
								Real_Clouds_Dn_P[n].startColor = Color.Lerp(Real_Clouds_Dn_P[n].startColor,Dusk_real_cloud_col_dn,0.5f*Time.deltaTime);
							}
							}
							if(Real_Clouds_Up_P!=null){
							for (int n = 0; n < Real_Clouds_Up_P.Length; n++){
								Real_Clouds_Up_P[n].startColor = Color.Lerp(Real_Clouds_Up_P[n].startColor,Dusk_real_cloud_col_up,0.5f*Time.deltaTime);
							}
							}
						}
					}
			...
			if(Season_prev != Season | !InitSeason){
					...
					if(Season == 3){
						...
						FallingLeaves_OBJ[n].SetActive(true);// Later change to instantiate the effect and destroy it
									FallingLeaves_P[n].maxParticles = 300;
								}
								}
...
//						}
							...
							if(Star_particles_OBJ_P!=null){
								if(Star_particles_OBJ_P.maxParticles>5){
									Star_particles_OBJ_P.maxParticles -=5;
								}else{
	...
	//						m_TintColor =  Color.Lerp (m_TintColor, new Color(0f,0,0,0) , 0.55f*UP_rate*Time.deltaTime);
			...
			if(USE_SKYBOX){
					...
					if(Weather == Weather_types.HeavyStormDark){
						...
						if(Star_particles_OBJ_P!=null){
							if(Star_particles_OBJ_P.maxParticles>5){
								Star_particles_OBJ_P.maxParticles -=10;
							}else{
								//Star_particles_OBJ.SetActive(false);
							}
						}
							...
							if(MoonObj.activeInHierarchy){
								MoonObj.SetActive(false);
										...
										//if(Current_Time > 9 & Current_Time <23){
												if(Application.isPlaying){
													MAIN.intensity = Mathf.Lerp(MAIN.intensity,Max_sun_intensity,To_max_intensity_speed*Time.deltaTime);
												}else{
													if(MAIN.intensity != Max_sun_intensity){
														MAIN.intensity =Max_sun_intensity;
													}
												}
										}else{
												if(Application.isPlaying){
...
//						}
			...
			if(!USE_SKYCUBE){	
				//v1.5
				if(skyMat != null){
					Assign_Material_Props(skyMat,Cam_tranform);//transform);
				}
				if(skyboxMat != null){
					Assign_Material_Props(skyboxMat,Cam_tranform);//transform);
				}
			}
		...
		//END SKYBOX
			...
			//v1.5
			if(skyMat !=null){
				Assign_Material_Props(skyMat,SkyCam_transform);
			}
			if(skyboxMat !=null){
				Assign_Material_Props(skyboxMat,SkyCam_transform);
			}

			SkyCam.farClipPlane  = 6750;
			...
			if(Preset == 8){
				...
				if(skyboxMat.HasProperty("SunColor")){
					...
					if(is_duskNight){
						...
						skyboxMat.SetFloat("White_cutoff",  Mathf.Lerp(skyboxMat.GetFloat("White_cutoff"),1.2f,SpeedA*FinalSPEED));
//						skyboxMat.SetColor("SunColor",  Color.white);
//						skyboxMat.SetFloat("SunBlend",  66);
//						skyboxMat.SetFloat("White_cutoff",  1.2f);
					}
