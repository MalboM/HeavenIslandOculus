...
Shader "SkyMaster/SkyMasterShaderE_SKYBOX U5 DUAL" 
{ 
 ...
 Properties {
		...
		DualSunsFactors("Dual Suns Factors", Vector) = (0.3, 0.3, 18)
    }
...
//				}										
    	...
    	//		OUTPUT.pos = mul(UNITY_MATRIX_MVP, v.vertex);
    			OUTPUT.pos = mul(UNITY_MATRIX_MVP, v.vertex);
    			//OUTPUT.pos.xy += _Obliqueness.xy * OUTPUT.pos.w;
...
//			half3 sun = (IN.normalAndSunExp.w > 0) ? pow(max(0.0, dot(normal, sunDir)), IN.normalAndSunExp.w) : 0.0;
      ...
      float3 NEW_D = normalize(IN.v3Direction);
				...
				//  float3 Scolor = (getRayleighPhase(fCos2)+(( 1.5 * ((1.0 - g2) / (2.0 + g2)) * (1.0 + fCos2M) / pow(1.0 + g2 - 2.0*g*fCosM, 1.5) ))) * IN.c0 + getMiePhase(fCos, fCos2, g, g2) * IN.c1;
				 ...
				 //DUAL
				 fCos = dot(-v3LightDirMoon, NEW_D);
				 fCos2 = fCos*fCos;
				 Scolor = DualSunsFactors.x*Scolor 
				 + DualSunsFactors.y*(getRayleighPhase(fCos2)+(( DualSunsFactors.z * ((1.01 - g2) / (1.2 + g2)) * (2.0 + fCos2) / pow(1.03 + g2 - 2.01*g*fCos, 1.4) ))) * IN.c0 +1*getMiePhase(fCos, fCos2, g, g2) * IN.c1;
				//------------------------------- INTENS ------------------------------------------------------------- HALO -----
