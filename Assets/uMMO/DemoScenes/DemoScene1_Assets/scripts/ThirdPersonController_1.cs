...
public class ThirdPersonController_1 : MonoBehaviour
{
	...
	// Handle rotation here to ensure smooth application.
	{
		...
		//rotationAmount = Input.GetAxis ("Horizontal") * turnSpeed * Time.deltaTime; //uMMO: This was the original line
	...
	// Handle movement here since physics will only be calculated in fixed frames anyway
	{
		...
		if (grounded)
		{
			...
			if (uMMO_NetObject.ME(this).GetButton("Jump"))
			//if (Input.GetButton ("Jump")) //uMMO: This was the original line
			// Handle jumping
			{
				//ThirdPersonCamera_smooth.activated = false;
				if (uMMO.get.architectureToCompile == uMMO_Architecture.Client) { //we intervene here, because having rigidbody-gravity active on the client really gives us annoying effects while jumping.
			...
			{
				//ThirdPersonCamera_smooth.activated = true;
				if (uMMO.get.architectureToCompile == uMMO_Architecture.Client) { //reset of above intervention
					target.constraints = RigidbodyConstraints.FreezeRotation;
					target.useGravity = true;
				}
				
				Vector3 movement = (uMMO_NetObject.ME(this).GetAxis("Vertical")) * target.transform.forward;
				//Vector3 movement = Input.GetAxis ("Vertical") * target.transform.forward; //uMMO: This was the original line
				
				float appliedSpeed = walking ? speed / walkSpeedDownscale : speed;
					// Scale down applied speed if in walk mode
				
				if (uMMO_NetObject.ME(this).GetAxis("Vertical") < 0.0f)
				//if (Input.GetAxis ("Vertical") < 0.0f) //uMMO: This was the original line
