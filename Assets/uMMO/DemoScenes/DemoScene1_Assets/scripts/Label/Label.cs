	...
	void OnGUI ()
	{
		...
		if (guiCamera != null) {
			...
			if (GetComponent<Collider>() != null)
				worldPosition = GetComponent<Collider>().bounds.center + Vector3.up * GetComponent<Collider>().bounds.size.y * 0.5f;
			else
				worldPosition = GetComponent<RacerNPC>().theRenderer.bounds.center + Vector3.up * GetComponent<RacerNPC>().theRenderer.bounds.size.y * 0.5f;
