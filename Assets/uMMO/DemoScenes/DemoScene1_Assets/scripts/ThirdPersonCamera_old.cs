...
public class ThirdPersonCamera_old: MonoBehaviour
{
	...
	private GUIText guitextReadjustCam;
		
		
	private const float movementThreshold = 0.1f, rotationThreshold = 9f;
		// Tweak these to adjust camera responsiveness
	...
	// Verify setup, initialise bookkeeping
	{
		...
		targetDistance = optimalDistance = ((camera.transform.position - target.transform.position).magnitude / 2);

		guitextReadjustCam = GameObject.Find ("cam_guitext").GetComponent<GUIText>();
		guitextReadjustCam.enabled = false;
	}
	...
	{
		if (
			(Input.GetMouseButton (0) || Input.GetMouseButton (2)) &&	// Act if a mouse button is down
	...
	void updateCamGUIText() {
			...
			// Stop rotating if we're within the threshold
		{
			...
			guitextReadjustCam.enabled = true;
		}
	}
