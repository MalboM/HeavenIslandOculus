...
public class BotControlScript : MonoBehaviour
{
	...
	void Update ()
	{
		bool isLocalPlayerClient = false;
		if (Network.isClient && GetComponent<uMMO_NetObject>().isLocalPlayer) {
			isLocalPlayerClient = true;
		}
			
		// if we hold Alt..
		if((Input.GetButton("Fire1") && isLocalPlayerClient) || (anim.GetBool("Firing" ) && !isLocalPlayerClient))
		{
			// ...set a position to look at with the head, and use Lerp to smooth the look weight from animation to IK (see line 54)
			anim.SetLookAtPosition(enemy.position);
			lookWeight = Mathf.Lerp(lookWeight,1f,Time.deltaTime*lookSmoother);
			if (isLocalPlayerClient) {
				anim.SetBool("Firing",true);
			}
		}
		// else, return to using animation for the head by lerping back to 0 for look at weight
		else if (isLocalPlayerClient || !anim.GetBool("Firing"))
		{
