...
void  UpdateSmoothedMovementDirection (){
	...
	if (getUserInput) {
		verticalInput = uMMO_NetObject.ME(this).GetAxisRaw("Vertical");
		horizontalInput = uMMO_NetObject.ME(this).GetAxisRaw("Horizontal");		
	}
	...
	if (grounded)
	{
		...
		// Pick speed modifier
		if (uMMO_NetObject.ME(this).GetButton ("Fire3"))
		{
...
void  ApplyGravity (){
	// Apply gravity
	if (getUserInput)
		jumpButton = uMMO_NetObject.ME(this).GetButton("Jump");
...
void  Update (){
	...
	if (getUserInput) {
		if (uMMO_NetObject.ME(this).GetButtonDown ("Jump"))
